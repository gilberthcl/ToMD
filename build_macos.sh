#!/bin/bash
# build_macos.sh — packages ToMd as a real macOS Application so you can
...
# (or just run ./build_app.sh, which auto-detects macOS and calls this.)
#
set -euo pipefail
...
cp "$HERE/tomd_cli.py" "$APP_PATH/Contents/Resources/"
...
#    just typing its name. It runs the interactive terminal front-end
#    (tomd_cli.py) with the same interpreter the app was built with.
mkdir -p "$BIN_DIR"
LAUNCHER="$BIN_DIR/ToMD"
APP_SCRIPT="$DEST/${APP_NAME}.app/Contents/Resources/tomd_cli.py"
cat > "$LAUNCHER" <<LAUNCH
#!/bin/bash
# Auto-generated by build_macos.sh — launches ToMd with the interpreter it was
